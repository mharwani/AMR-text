...
def make_vocabs(train_pairs):
    ...
    for snt,amr in train_pairs:
        amr_vocab.add_words(snt)
        amr_vocab.add_graph(amr.root)

    logging.info('amr (src) vocab size: %s, edge_vocab_size: %s', amr_vocab.n_words, amr_vocab.n_edges)
...
def encode_node(node, amr_vocab, encoder, child_sum, processed, outputs):
    ...
    ind = OOV_index
    if node.inst in amr_vocab.word2index:
        ind = amr_vocab.word2index[node.inst]
    ...
    for edge,child_node in node.child.items():
        ...
        if type(child_node) is str or type(child_node) is float:
            inp_child = OOV_index
            if child_node in amr_vocab.word2index:
                inp_child = amr_vocab.word2index[child_node]
            inp = torch.tensor([[inp_child]], device=device)
            outp, hidden = encoder(inp, None)
            outp2 = torch.cat(hidden, dim=2)
            outputs[0] = torch.cat((outputs[0], outp2))
        #amr nodes
        else:
        ...
        if edges is None:
            ...
            edges = torch.cat((edges, edge_tensor), 0)
            hidden_states0 = torch.cat((hidden_states0, hidden[0]), 0)
            hidden_states1 = torch.cat((hidden_states1, hidden[1]), 0)
    ...
    outp2 = torch.cat(hidden_final, dim=2)
    outputs[0] = torch.cat((outputs[0], outp2))   
    return output, hidden_final
    
def encode_amr(graph, amr_vocab, encoder, child_sum, max_in_length):
    outputs = [torch.zeros(1, 1, encoder.hidden_size*2, device=device)]
    ...
    for i in range(pad):
        en_outputs = torch.cat((en_outputs, torch.zeros(1, 1, encoder.hidden_size*2, device=device)))
...
def main():
    ...
    if load_state_file is not None:
        ...
        amr_vocab = make_vocabs(train_pairs)
    ...
    while iter_num < args.n_iters:
        ...
        target_snt = tensors_from_batch(amr_vocab, random_pairs)
