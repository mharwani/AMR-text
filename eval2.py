...
from train import encode_amr, SOS_index, EOS_index, EOS_token, AMRVocab, Vocab

device = torch.device("cuda")



def translate(encoder, child_sum, decoder, input_amr, amr_vocab, en_vocab, max_length):
    """
    ...
    with torch.no_grad():
        ...
        for di in range(cutoff):
            ...
            if topi.item() == EOS_index:
                ...
                decoded_words.append(en_vocab.index2word[topi.item()])
...
def translate_amrs(encoder, child_sum, decoder, pairs, amr_vocab, en_vocab, max_length, max_num_sentences=None):
    output_sentences = []
    for snt,amr in pairs[:max_num_sentences]:
        output_words = translate(encoder, child_sum, decoder, amr, amr_vocab, en_vocab, max_length)
...
def translate_random_amr(encoder, child_sum, decoder, pairs, amr_vocab, en_vocab, max_length, n=1):
    for i in range(n):
